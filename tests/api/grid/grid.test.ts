...
it('Should get grids', async () => {
    ...
    assert.notOk(user2grids.length);

    const user3grids = await user3.prisma.grid.findMany({ include: GridInclude });
    assert.equal(user3grids.length, 1);
    assert.equal(user3grids[0].columns, 6);
    const card = user3grids[0].elements[0];
    assert.equal(card.type, 'Card');
    assert.equal(card.card?.footer?.button?.text, 'Create New Order');
    assert.equal(user3grids[0].elements[1].card?.footer?.progress?.value, 25);
    const tabs = user3grids[0].elements[3];
